...
export function insightForStep(decisionPointId, band) {
...
export function buildJourneyReflection(state) {
    ...
    if (poorCount > 0) {
        parentCoaching.unshift("Your child practiced repair after a harder choice — that resilience is more valuable than a flawless path.");
    }
    const summary = strongCount >= Math.max(1, events.length - 1)
    ...
    return {
        ...
        stepInsights,
        parentCoaching,
        closingNote: "You are not alone in this. Small daily conversations about feelings build lifelong emotional health — one kind step at a time.",
    };
}
export function childFacingLine(insight, companionName) {
