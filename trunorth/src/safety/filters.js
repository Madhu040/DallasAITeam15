export function sanitizeChildInput(input) {
    return input
        .replace(/<[^>]*>/g, "")
        .slice(0, 500)
        .trim();
...
export function filterInput(input) {
    ...
    for (const p of JAILBREAK_PATTERNS) {
        if (p.test(text))
            return { allowed: false, safetyFlag: "jailbreak" };
    }
    for (const kw of DISTRESS_KEYWORDS) {
        if (text.toLowerCase().includes(kw))
            return { allowed: false, safetyFlag: "distress" };
    }
    for (const p of PII_PATTERNS) {
        if (p.test(text))
            return { allowed: false, safetyFlag: "pii" };
    }
    for (const word of PROFANITY) {
        if (text.toLowerCase().includes(word))
            return { allowed: false, safetyFlag: "profanity" };
    }
