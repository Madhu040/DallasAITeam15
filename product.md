...
| Live model verification — ADR-004 pinned `claude-haiku-4-5-20251001` is verified only at the config/plumbing level; no `ANTHROPIC_API_KEY` was available, so a real Haiku 4.5 call against the §9.4 JSON contract is unproven | Build | ⬜ owed before any keyed deploy |
...
| Time the golden path against the §27 DoD 3-minute budget (never measured; ch1 grew 2→5 DPs in Phase 2) | (unassigned) | ⬜ cheap + stage-critical |
| Playwright offline assertion for demo mode (§27 DoD item 3) — `test:e2e` script + devDependency exist, no `tests/e2e/` | (unassigned) | ⬜ cheap + stage-critical |
