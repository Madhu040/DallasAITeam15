...
| Production art — **AI PNGs prepped & validated 2026-07-19.** 31 files in `public/assets/{characters,backgrounds,objects}/` (Nano Banana/Midjourney), **30 of 31 ready**: correct lowercase names, backgrounds opaque (fine), objects + characters transparent, **7.2 MB total (under the 15 MB §19 budget)**. **1 blocker left: `deer.png` is still an opaque white box** (missed background removal) — non-blocking since the manifest falls back to the code-drawn sprite. **PNGs still uncommitted** (add them in the manifest change once verified in `dist`). **NEXT: the `assetRef → file` manifest (spec §10.3) is NOT built** — this is the immediate task; it must fall back to the existing code-drawn SVG cast / emoji props so the offline demo never breaks on a missing file. | (build) | 🟨 art ready, **manifest is the next build** |
