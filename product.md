...
> 13 discovery dialogs + Kindness Sparks), per-chapter celebration, a **following zoom
> camera** (§5), and the **art asset manifest** — real AI backgrounds + props now render
> with a live fallback. 135 unit + 18 e2e green.
>
...
> >    31 PNGs committed. Only `avatar` stays on SVG (skin-tone choice). **→ next real task is #2.**
...
| Ch.2 has the same sparse-collectible problem Ch.1 had — the §7.6 spark pass has not been applied to w1–w7 | (unassigned) | ⬜ new 2026-07-19 |
| Ch.2 has no explore loop — w1–w7 are still spawn→hitbox→answer, the shape ch1 just moved away from | (unassigned) | ⬜ new 2026-07-19 (supersedes the earlier "sparse collectibles" row) |
| Production art — **manifest built & fully wired 2026-07-19.** `src/content/assetManifest.ts` (spec §10.3) swaps PNGs in for placeholders with an `onerror` live-fallback. **Live in the demo:** all 5 backgrounds, all 17 props, and 8 of 9 characters (Flicker, Jamie, the deer, etc.) — verified in-browser with zero fallbacks firing. Character art was re-cut to tight transparent cutouts (the deer's old opaque box is fixed). **Only `avatar` stays on the tone-aware SVG** — deliberate, so the child's 5-way skin-tone pick isn't erased by a single `nova.png`. 31 source PNGs committed (7.2 MB, bundle 8.03/15 MB). | (build) | ✅ art live (avatar SVG by design) |
