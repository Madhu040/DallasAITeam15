...
| Production art — **first AI PNGs received 2026-07-19** (18 files from Nano Banana/Midjourney in `public/assets/{characters,backgrounds,objects}/`) but **not yet usable**: filenames don't match the manifest convention (spaces/typos), the player avatar **Nova is missing**, object PNGs are multi-item **sheets not cut-outs**, character backgrounds are **opaque white not transparent** (corner pixels ~`[250,250,250,255]` → white box on the world), and the set totals **~17 MB vs the 15 MB §19 budget**. The `assetRef → file` manifest that renders them (spec §10.3) is also **not built**. PNGs are intentionally **left uncommitted** until prepped. | Ermoni + Gabby / (build) | 🟨 art received, needs prep (rename, cut, de-background, optimize) + manifest |
