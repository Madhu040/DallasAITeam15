...
| Hosted deploy (client + Hono API) | Jose | 🟨 **Deployed 2026-07-20**: https://guardiandragon.vercel.app is live. **Static client works fine** (verified 200 OK). **`/api/*` was crashing** — `POST /api/companion` returned `500 FUNCTION_INVOCATION_FAILED`. Root-caused precisely (not just suspected): `server/index.ts` imports `server/db/migrate.ts` unconditionally at module scope, and that module runs `mkdirSync`/`new Database(...)` **at import time** — so *every* `/api/*` route crashes on cold start regardless of whether that route touches the DB (confirmed `routes/companion.ts` itself never references `db`; the crash was pure collateral damage from the bootstrap). Vercel Node Functions have no writable disk outside `/tmp`, and the default `databasePath` was `./data/trunorth.db`. **Fixed in code** (`server/config.ts`): defaults to `/tmp/trunorth.db` when `process.env.VERCEL` is set (Vercel sets this on every deployment automatically — no dashboard env var needed), unchanged `./data/trunorth.db` locally; an explicit `DATABASE_PATH` still overrides either way. Verified the branch logic directly (`VERCEL=1` → `/tmp/trunorth.db`; unset → `./data/trunorth.db`) since the local `.env` sets `DATABASE_PATH` explicitly and would otherwise mask the fallback in a naive test. **Two more layered bugs found after redeploying, each surfacing only once the previous one was fixed:** (1) a stale Vercel dashboard env var `DATABASE_PATH=tmp/trunorth.db` (missing its leading slash) was overriding the code fix entirely — an explicit env var always wins over the code's conditional default, so the fix above had zero effect until the owner corrected/removed that var. Confirmed precisely from the redeploy's own stack trace: `ENOENT ... mkdir 'tmp'` — a *relative* path, only possible if something upstream of the code default supplied `tmp/...` without the leading `/`. (2) Once that was fixed, requests stopped crashing on import but started **hanging indefinitely** (verified: 3 separate requests, 15–20s each, zero response) — a *new* stack trace showed `TypeError: this.raw.headers.get is not a function` inside Hono's CORS middleware. Root cause: `api/[[...route]].ts` used `hono/vercel`'s `handle`, which is a bare `(req) => app.fetch(req)` that assumes Vercel already hands the function a Web-standard `Request` — true for the **Edge** runtime, not for the **`nodejs`** runtime this function requires (for `better-sqlite3`'s native binary). A `nodejs` runtime function gets Node's classic `(req, res)` objects instead, whose `req.headers` is a plain object with no `.get()` — exactly matching the crash. **Fixed**: switched to `@hono/node-server/vercel`'s `handle` (the package was already a dependency, unused for this) — its `getRequestListener` is the adapter actually built to convert Node's raw req/res into a real Request/Response pair. **Verified locally, redeployed — `/api/health` confirmed live** (`200`, real JSON), proving the adapter switch fixed bug #2. **But a fourth, distinct bug immediately surfaced**: every `POST` request (even a bodyless one — isolated by testing, not guessed) hung indefinitely, while `GET /api/health` stayed fast. Root cause: Vercel's Node Functions auto-parse a JSON body onto `req.body` for convenience (documented, standard behavior), which drains the underlying raw request stream to do it — then `@hono/node-server`'s adapter tried to read that *same* stream itself to build the Fetch `Request` body, hanging forever on a stream that had nothing left to give. **Fixed**: replaced both prior adapters with a small hand-written one in `api/[[...route]].ts` that builds the Request directly — using Vercel's pre-parsed `req.body` when present (no second stream read) and falling back to reading the raw stream only when it isn't (e.g. local `vite dev`, which never populates `req.body`). **Verified both paths locally before redeploying again**: a plain local `http.createServer` test (raw-stream fallback, mirrors local dev) and a second test that manually drains the request stream and sets `req.body` *before* invoking the handler — deliberately reproducing Vercel's own behavior rather than just trusting the theory — both returned a fast `400` with no hang. **Redeployed and confirmed live: `GET /api/health` and `POST /api/companion` (both an invalid-body `400` and a real valid-payload `200` with a genuine scored response) all worked, fast, no hang** — bugs #1-4 are genuinely fixed. **A fifth, different-shaped bug immediately surfaced testing the DB-backed routes**: `POST /api/auth/register` and every other multi-segment path (`/api/auth/me`, `/api/progress/:id`, `/api/together/...`) returned Vercel's own branded `404 NOT_FOUND` — the request never reached the function at all (same `cle1::` request-ID signature the earlier crash pages had). Every single-segment path (`/api/health`, `/api/companion`) kept working. Verified this wasn't the code: the identical handler served `/api/auth/register` correctly locally (`201`, real JWT, real DB write). Ruled out the Vercel project's Root Directory setting (checked the dashboard directly — correctly `trunorth`; also single-segment paths already proved the function itself deploys and runs fine). First guess was that `api/[[...route]].ts`'s double-bracket "optional catch-all" was a Next.js-only convention Vercel doesn't honor outside Next.js — renamed to single-bracket `api/[...route].ts` ("one or more segments") and redeployed. **That guess was wrong**: identical failure persisted. Dug further before guessing again: `/api/nonsense` (a made-up *single* segment) reached the function and got Hono's own plain `404 Not Found` — but `/api/health/` (same working path, trailing slash), bare `/api`, and *every* multi-segment path (`/api/auth/register`, `/api/foo/bar`, …) all got Vercel's own branded `NOT_FOUND` — never reaching the function, regardless of which bracket convention named the file. Dynamic-segment file routing genuinely wasn't reliable on this project. **Fixed with the more explicit, bulletproof mechanism instead**: replaced the dynamic filename entirely with a static `api/index.ts` (no dynamic segment) plus a `vercel.json` rewrite (`/api/:path*` → `/api`) that routes every `/api/*` request to this one function regardless of depth — a Vercel *rewrite* (unlike a redirect) preserves the original request path for the destination handler, so the app's own internal Hono routing is unaffected. Verified the handler code itself still works after the rename (local `GET /api/health` → `200`, `POST /api/auth/register` → `201`); the routing-layer fix itself is, by definition, unverifiable outside Vercel's actual platform. ⚠️ **Needs a seventh redeploy to confirm** — the fifth code fix in this investigation, and the second attempt specifically at the multi-segment routing bug. **Known residual limitation, unrelated to any of the above and not a bug**: `/tmp` is writable but not persistent across cold starts or shared between concurrent instances, so parent auth / child profiles / progress / Play Together rooms can still appear to "reset" — real persistence needs a hosted DB (Postgres/Supabase/Turso, same open question as the Supabase row above). `?demo=1` still sidesteps all of this by running fully client-side |
...
| **Mobile / touch device support** — scoped 2026-07-20, **built and verified same day**. All 5 plan items landed. (1) **Touch input**: `src/input/TouchControls.ts` (new) mounts a **floating** virtual joystick + a fixed interact button, feeding `InputController`'s existing `getMoveVector()`/`consumeInteract()` via two new methods (`setTouchVector`, `queueTouchInteract`) — keyboard and touch are two input sources summed then clamped to the unit circle, neither replacing the other. **Revised same day, after the owner asked for a more finger-first feel than a fixed on-screen gamepad**: the joystick has no resting position — a full-stage `.touch-move-zone` catches a touch anywhere except the interact button's own footprint (which paints on top of it in DOM order, so no exclusion rectangle is needed) and the joystick appears centered right where the finger landed, opacity-faded in via a CSS `.active` class. Considered and deliberately not built: tap-to-move (would need a real "walk toward point X,Y" mode plus pathfinding — `WorldRuntime` only has per-frame velocity + AABB collision today, so a tap across a pond could get the avatar stuck) and auto-interact-on-proximity (would remove the deliberate "an explicit press is required, walking near a trigger alone never fires it" behavior, a design choice not a gap). Mounted only when `matchMedia('(pointer: coarse)')` matches (checked in `WorldRuntime.renderTouchControls`, called from the same place as the existing `.move-hint`), so desktop gets zero DOM/listeners for this, not just a CSS-hidden control. (2) **Portrait rotate-prompt**: pure CSS (`index.html` + `global.css`), gated by the same `(pointer: coarse) and (orientation: portrait)` media query — no JS orientation listener needed, reacts live as the device rotates. (3) **`--px` scaling measured, not assumed, at a real phone width**: new e2e test boots to gameplay at 844×390 and re-runs the projector suite's own checks (no page overflow, stage holds 16:9, HUD unclipped, decision text ≥12px, hit target ≥64px) — confirming the formula that was only proven from 1024px up now holds at the other end too. (4) **Audio unlock**: `unlockAudioOnFirstGesture()` in `sfx.ts`, wired to a one-time `document` `pointerdown` listener in `main.ts` — silently (volume-zeroed) plays-then-pauses every cue once on the very first tap anywhere, satisfying iOS Safari's gesture requirement without knowing which button starts the game. (5) **Mobile Playwright project**: `playwright.config.ts` gained a `mobile` project (`devices["Pixel 5"]` — Chromium/Android rather than an iPhone preset, since iOS presets default to WebKit, a second browser engine this repo doesn't otherwise install; real iOS Safari verification stays a manual device task, same caveat the projector suite already carries for the real venue). New `tests/e2e/mobile.spec.ts` (5 tests): portrait shows the prompt, landscape shows correctly-sized controls, **a full touch-driven walk-to-real-decision** (joystick drag + interact tap reaches the same `dp_leftout_bench` trigger `camera.spec.ts` proves reachable by keyboard, using the identical spawn-to-trigger geometry), the scaling-measurement test above, and a reverse check that desktop gets none of this UI. **One real debugging finding**: `page.mouse` doesn't reliably produce `pointerdown`/`pointermove` under Playwright's touch-emulated (`hasTouch: true`) context — verified by hand (a drag via `page.mouse` left the avatar's DOM position unchanged; the identical gesture via direct `PointerEvent` dispatch moved it) — so the test helpers dispatch pointer events directly instead. 170 unit (169→170, new sfx test) + 24 e2e (19→24) green; typecheck/lint/validate/build/audit clean; bundle unchanged at 9.91 MB. Desktop's existing 19 e2e tests are unaffected (verified — `chromium` project explicitly ignores `mobile.spec.ts`) | (unassigned) | ✅ done 2026-07-20 |
...
| Date | Change |
|---|---|
| 2026-07-20 | **The bracket-rename fix for bug #5 didn't work — dug further, found the real shape of the problem, and replaced dynamic-segment routing entirely with a `vercel.json` rewrite.** Redeployed `api/[...route].ts` (single-bracket rename) and the multi-segment 404s persisted identically. Rather than guess again, isolated the actual failure boundary: `/api/nonsense` (a made-up *single* segment) reached the function and got Hono's own plain `404 Not Found`, proving the function itself was healthy — but `/api/health/` (the same working path, just a trailing slash), bare `/api`, and every multi-segment path all got *Vercel's own* branded `NOT_FOUND`, never reaching the function at all, regardless of bracket convention. So it was never about "segment count" — dynamic-segment file-based routing simply wasn't reliable past a bare single clean segment on this project, for either bracket syntax. Replaced it with the more explicit, standard mechanism: a static `api/index.ts` (no dynamic filename at all) plus a `vercel.json` rewrite (`/api/:path*` → `/api`) that forces every `/api/*` request to this one function regardless of depth — a Vercel *rewrite*, unlike a redirect, preserves the original request path for the destination handler, so the app's own internal Hono routing (which reads `req.url`) is unaffected. Verified the handler code itself still works after the rename (local `GET /api/health` → `200`, `POST /api/auth/register` → `201`); typecheck/lint/170 unit/validate:content/build/24 e2e all clean. The routing-layer fix itself remains, by definition, unverifiable outside Vercel's actual platform — no amount of local testing can reproduce Vercel's own routing decision, which is exactly the class of bug this has been from the start. ⚠️ Needs a seventh redeploy to confirm — the fifth code fix in the same investigation, second attempt at this specific bug. |
| 2026-07-20 | **Bugs #1-4 in the Vercel API investigation all confirmed genuinely fixed live — then a fifth, differently-shaped bug turned up: the catch-all file used a Next.js-only convention.** Redeployed the custom-handler fix (bug #4) and this time actually verified the wins: `GET /api/health` and `POST /api/companion` (both an invalid-body `400` and a real valid-payload `200` with a genuine scored response) all worked, fast, no hang. Then tested the database-backed routes for the first time in this investigation and found something new: `POST /api/auth/register` and every other multi-segment path (`/api/auth/me`, `/api/progress/:id`, `/api/together/...`) returned Vercel's own branded `404 NOT_FOUND` — the request never reached the function at all — while every single-segment path (`/api/health`, `/api/companion`) kept working. Verified this wasn't a code bug: the identical handler served `/api/auth/register` correctly locally (`201`, real JWT, real DB write). Checked the Vercel dashboard's Root Directory setting directly rather than guessing (correctly `trunorth`) before looking elsewhere. Root cause: the file was named `api/[[...route]].ts` — double-bracket "optional catch-all," a **Next.js-specific** routing convention. Plain Vercel Functions outside Next.js don't reliably treat it as a true wildcard; empirically it behaved like a single-segment matcher instead of "zero or more segments," which explains the exact split observed. Fixed by renaming to `api/[...route].ts` (single brackets — "one or more segments," matches any depth under `/api/`). Verified the handler code itself is unaffected by the rename (local `GET /api/health` → `200`, `POST /api/auth/register` → `201`) — but by definition, the actual routing-layer fix this targets can't be verified outside Vercel's own platform, unlike every previous fix in this investigation which at least had *some* local reproduction available. typecheck/lint/170 unit/validate:content/build/24 e2e all clean (one accessibility test flaked under full-suite load, confirmed pre-existing and unrelated — passed reliably in isolation, and this fix never touches client code at all). ⚠️ Needs a sixth redeploy to confirm — the fourth code fix in the same investigation. |
| 2026-07-20 | **Fourth bug in the Vercel API investigation: replaced both Hono Vercel adapters with a small hand-written one, after the previous fix confirmed live but immediately surfaced the next layer.** The `@hono/node-server/vercel` switch worked — redeployed, and `GET /api/health` came back `200` with real JSON, confirming bug #2 (the wrong adapter for a `nodejs`-runtime function) was actually fixed, not just locally plausible. But every `POST` request then hung indefinitely, isolated by testing (not assumed) to the *method*, not the body content — a bodyless `POST` hung identically to one with a JSON body, while `GET` stayed fast. Root cause: Vercel's Node Functions auto-parse a JSON body onto `req.body` for convenience (standard, documented behavior) — which drains the underlying raw request stream to do it. `@hono/node-server`'s adapter then tries to read that *same* stream itself to build the Fetch `Request` body, and hangs forever on a stream with nothing left to give. Fixed by writing a small custom handler in `api/[[...route]].ts` (~40 lines) that builds the `Request`/`Response` directly: uses Vercel's pre-parsed `req.body` when present (no second stream read), falls back to reading the raw stream only when it isn't (e.g. local `vite dev`, which never populates `req.body`). **Verified both code paths locally before redeploying again** — not just the happy path: one test uses a plain local `http.createServer` (mirrors local dev, exercises the raw-stream fallback), a second *deliberately reproduces Vercel's own behavior* by manually draining the request stream and setting `req.body` before invoking the handler, rather than trusting the theory on inspection alone. Both returned a fast `400` with zero hang. typecheck/lint/170 unit/validate:content/build/24 e2e all clean (client untouched). ⚠️ This is the third code fix in the same investigation; the first two were each locally sound and still needed the real Vercel platform to expose the next layer, so this one carries the identical caveat — needs a fifth redeploy to confirm. |
| 2026-07-20 | **Fixed the real cause of the Vercel `/api/*` hang: the wrong Hono adapter for a `nodejs`-runtime function.** Two bugs deep from the original DB-path fix, each only visible once the prior one was cleared. First, a stale Vercel dashboard env var `DATABASE_PATH=tmp/trunorth.db` (missing its leading slash) had been silently overriding the earlier code fix the whole time — an explicit env var always wins over a conditional default, so nothing in the earlier fix could have worked until that var was corrected or removed; caught from the redeploy's own stack trace (`ENOENT ... mkdir 'tmp'` — a relative path, the unmistakable fingerprint of a value missing its leading `/`). Once that was cleared, requests stopped crashing but started **hanging indefinitely instead** (verified directly: 3 separate curls, 15–20s each, zero response, not a fast error) — a fresh stack trace showed `TypeError: this.raw.headers.get is not a function` inside Hono's CORS middleware. Root cause: `api/[[...route]].ts` used `hono/vercel`'s `handle`, a bare `(req) => app.fetch(req)` that assumes Vercel already provides a Web-standard `Request` — true only for the **Edge** runtime; the `nodejs` runtime this function requires (for `better-sqlite3`) instead hands Node's classic `(req, res)`, whose `req.headers` has no `.get()`. Fixed by switching to `@hono/node-server/vercel`'s `handle` — the package was already an installed dependency, just never used for this entry point — whose `getRequestListener` is the adapter actually built to bridge Node's raw req/res into a real Request/Response pair. **Verified locally before asking for a fourth redeploy, not just by reading the diff**: ran the *exact* exported handler through a real `http.createServer` + real `fetch()` calls — `GET /api/health` → `200` with the expected JSON, `POST /api/companion` → a proper `400` validation error (the exact route that used to crash) — confirming the CORS middleware and routing both work end to end outside Vercel's platform. typecheck/lint/170 unit/validate:content/build/24 e2e all still clean (client code untouched by this fix, so the e2e suite result was expected, not just assumed). ⚠️ Still needs that redeploy to confirm against the actual Vercel platform — the only environment that reproduced any of these three bugs; thorough local verification is not a substitute for it. |
| 2026-07-20 | **Joystick switched from fixed-corner to floating, after owner feedback that a pinned on-screen gamepad felt less natural than "just use your finger."** `TouchControls.ts`: removed the joystick's `left`/`bottom` CSS anchor; a new full-stage `.touch-move-zone` catches the first touch anywhere (except the interact button's own footprint, which paints on top of it in DOM order — no exclusion math needed) and the joystick now appears centered wherever that touch landed, fading in via a CSS `.active` class rather than always being visible. The interact button stays fixed bottom-right on purpose — a single reliable, always-in-the-same-place tap target, unlike the joystick which benefits from following the finger. Considered two bigger alternatives and deliberately didn't build them, recorded so the reasoning survives a context reset: **tap-to-move** (needs a real "walk toward point X,Y" mode plus pathfinding — today's `WorldRuntime` only does per-frame velocity + AABB collision, so a tap across the pond could strand the avatar with no route around it) and **auto-interact-on-proximity** (would remove the existing, deliberate "an explicit press is required — walking near a trigger alone never fires it" behavior, which reads as a design choice protecting against accidental triggers, not a gap to close). Updated `tests/e2e/mobile.spec.ts`'s touch-drag helper to match: the joystick has no rect to read before first touch anymore, so the test now dispatches its own chosen origin point at `.touch-move-zone` instead of reading `.touch-joystick`'s (now meaningless-until-touched) bounding box. **Padded the walk-loop's lap budget 18→24** after it flaked once under full-suite load despite passing reliably alone (timing-tight, not a movement regression — confirmed by rerunning the single test in isolation before touching the loop). Ran the full suite twice after the fix for stability, not just once. 170 unit + 24 e2e still green across both runs; typecheck/lint/validate/build clean; bundle unchanged. |
| 2026-07-20 | **Mobile/touch support built — all 5 scoped plan items, same day.** See the gap-table row above for the full breakdown. Highlights: a virtual joystick + interact button feed the *same* `InputController` interface the keyboard already drives (two input sources summed, not a parallel code path), gated behind `matchMedia('(pointer: coarse)')` so desktop gets zero new DOM/listeners, not just hidden ones; a pure-CSS portrait rotate-prompt needing no JS orientation handling; a new e2e test proves the projector fix's `--px` scaling formula holds at a real 844×390 phone width, not just assumed from the 1024px+ range it was actually measured at; an audio-unlock hook satisfies iOS Safari's gesture requirement by silently priming every cue on the very first tap anywhere; and a new `mobile` Playwright project (Pixel 5/Chromium, not an iPhone preset — those default to WebKit, a browser engine this repo doesn't otherwise install) with 5 new tests, including a full touch-driven walk-to-decision that reaches the same trigger `camera.spec.ts` already proves reachable by keyboard. **Real debugging finding along the way**: `page.mouse` doesn't reliably fire `pointerdown`/`pointermove` under Playwright's touch-emulated context — confirmed by hand before assuming the test framework rather than the app was at fault (a `page.mouse` drag left the avatar's DOM position unchanged; the identical gesture via direct `PointerEvent` dispatch moved it) — so the test helpers dispatch events directly. 170 unit (+1) + 24 e2e (+5) green; existing 19 desktop e2e tests confirmed unaffected; typecheck/lint/validate/build/audit clean; bundle unchanged at 9.91 MB. |
