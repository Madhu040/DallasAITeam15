...
| Engineering spec v3.0 pins **Vite 8.x**; build runs `vite ^6.2.2` (6.4.3) — two majors behind and unpinned (Consolidated Appendix F) | (unassigned) | ⬜ needs a reviewed dependency bump + regression run, not a silent edit |
| Engineering spec v3.0 requires **Claude Haiku 4.5 with a production-pinned dated ID + lifecycle policy**; build uses `claude-3-5-haiku-latest` — a previous-generation model behind a floating alias (`server/config.ts`, `.env.example`) | (unassigned) | ⬜ the floating alias is exactly what the spec's lifecycle policy exists to prevent |
| Engineering spec v3.0 prefers **Node 24.x LTS** (22.x = compat fallback); repo is on the fallback (`.nvmrc` 22, `engines: >=20`) | (unassigned) | 🟨 acceptable per spec, but a deliberate call |
...
| Engineering spec v3.0 Part II governance artifacts absent from the repo — gap register, ADRs (it references ADR-001 for the Vite pin), risk register, Definition of Done, confidence matrix | Madhu | ⬜ `[MVP]` governance per v3.0 |
