...
| Last updated | 2026-07-23 (speech captions now also cover the check-in results screen — a third, previously-untouched caller — plus a voice-toggle troubleshooting note; see §5/§6) |
...
│   ├── routes/checkin.ts      # ✅ POST /api/checkin/score — LLM-backed check-in scoring (ADR: see §3.6a)
...
│   ├── companion/CheckinScorer.ts     # ✅ Live/Demo check-in batch scorer + scoreCheckinBatch (§3.6a)
...
│   └── ui/                    # ✅ GameView, screens, togetherScreens, childrenScreen, auth (Supabase Auth), captions
├── supabase/migrations/       # ✅ 0001_children_progress.sql — child_profiles/progress/audit_logs + RLS (paste into SQL Editor)
├── tests/unit/                # ✅ 197 tests — engine (13) + grid (8) + checkin (9) + stageObjects (10) + phase1 (6) + phase2 (7) + phase3 (7) + phase4-errors (10) + sparks (10) + exploreLoop (7) + redteam (51) + supabaseAuth (4) + authGate (3) + checkinScore (11) + checkinScorer (7) + others
...
- `childrenScreen.ts` — **new (2026-07-20, ADR-003).** `renderChildren(container,
  ...
  /api/children`), add-child form → `POST /api/children`, Sign Out, generic Continue →
  hub. **Each child row also has its own "Play as {name}" button** (`onContinue(childName)`)
  — this is the one place a real child name enters `gameState.profile.childDisplayName`
  for a logged-in session (`main.ts`'s `"children"` case sets it before navigating to hub);
  without it every session defaults to the hardcoded placeholder `"Nova"` from
  `createInitialGameState`, which the check-in greeting (§3.6a) would otherwise address by
  name. Reached via `main.ts`'s new `"children"` `AppScreen`.
...
- `captions.ts` — **new (2026-07-20); timing bug fixed same day; repositioned
  bottom-right → bottom-center 2026-07-21.** Persistent, translucent, click-through captions **pill** fixed
  to the bottom-center of the viewport (`.speech-captions` in `global.css`, small/rounded/
  translucent — movie-subtitle style, deliberately not a big card — `z-index: 101` above
  modal overlays so it stays legible even during a dialog pop-up), showing text whenever a
  character speaks: the
  companion's speech bubble line (`GameView.ts`'s bubble render, prefixed with
  `state.profile.companionName`) and every page of a stage-object NPC dialog
  (`renderDialogOverlay`, prefixed with that page's `speaker`). Independent of the TTS
  voice toggle (`src/audio/speech.ts`) — a session with voice off still gets captions.
  `showCaption(speaker, text, persistent?)` / `clearCaption()`; lives outside `#app`
  (appended once to `document.body`) since every screen fully replaces `#app`'s contents
  on render.
  ⚠️ **First-ship bug, found via manual report ("captions aren't showing"):** the initial
  version cleared the caption whenever a given `renderGameView` call didn't currently show
  a bubble/dialog. That's wrong for the companion bubble specifically —
  `SceneEngine.resolveDecision` sets phase `consequence` and then, synchronously with no
  delay, `exploring` (same function, no intervening await beyond a `store.save`), so the
  bubble's one visible render pass could last only milliseconds — nowhere near long enough
  to read, and often already gone by the very next render. **Fix:** `showCaption` now takes
  a `persistent` flag. Non-persistent (the bubble, the common case) starts a
  `setTimeout` — `appConfig.timing.captionMinVisibleMs` (new, default 4500ms,
  `VITE_CAPTION_MIN_VISIBLE_MS`) — that's the actual minimum a line stays up, decoupled
  from the underlying phase's real (and much shorter) lifetime; a new call resets the
  timer. Persistent (stage-object dialogs, which stay open until the player dismisses
  them — a real, stable "still speaking" condition) skips the timer entirely and is
  cleared explicitly instead, from `main.ts`'s `closeDialog()`. The old per-render
  `captionSpoken`/blanket-clear logic was removed from `renderGameView` entirely — it was
  the actual bug, not a redundant safety net. `navigate()` still clears on leaving the game
  screen (alongside `stopSpeaking()`), unchanged. **Deliberately does not caption** the
  decision-prompt/discuss-prompt accessibility scripts (`buildOverlayScript`) — TTS-only
  narration of content already fully visible as structured UI (heading + option buttons).
  **Verified in-browser** (Chrome automation, MutationObserver-instrumented, not just
  screenshots): the bubble caption appeared instantly and stayed `visible` for the full
  ~4.5s window before auto-hiding (confirmed timestamps, not just a screenshot glance —
  the first debugging pass falsely looked "still broken" purely because the poll started
  late enough that the *correctly-working* timer had already expired); the dialog caption
  stayed visible past 7s while the dialog remained open and cleared the instant it closed.
  **Repositioned bottom-right → bottom-center** on a second user report of "still not
  showing" (§6 changelog) — re-verified from a completely fresh `localStorage.clear()` +
  reload playthrough with the same `MutationObserver` instrumentation: the scene-opening
  companion line and a decision's consequence line both correctly showed at the new
  position and auto-hid on schedule.
  **Third gap closed 2026-07-23:** the pre-level check-in's results screen (§3.6a's
  greeting/`checkinCompanionLine`) was still a silent gap — it's rendered by `screens.ts`,
  a completely separate screen from `GameView.ts`, which the caption wiring had never
  touched. `renderResult()` now calls `showCaption(companionName, lineText, true)`
  (persistent — this screen stays up until the child taps "Start", the same kind of
  stable condition as a stage-object dialog), with an explicit `clearCaption()` on that
  button before `onDone(result)` fires. Verified in-browser: caption held for the full
  ~23s of manual interaction on that screen (no premature timeout, confirming the
  `persistent` path, not the timer path, was used) and cleared the instant "Start" was
  clicked, with no stale leftover text once gameplay began.
...
### 3.6a Check-in LLM scorer (`src/companion/CheckinScorer.ts`) — new 2026-07-20
✅ Implemented — the first LLM-backed scoring path for the pre-level check-in (previously
100% offline keyword heuristic). Mirrors `CompanionClient`'s Live/Demo split but scores the
**whole check-in as one batch call at the end**, not per-question, since check-in already
shows no per-question feedback UI. **Auth-gated (2026-07-20):** the server only spends the
Anthropic key on a logged-in parent session (same rule as `/api/companion`, §3.11) — the
guest path (no login) always gets the offline heuristic regardless of whether a key is
configured; see [safety-companion-pipeline.md](./docs/context/safety-companion-pipeline.md).
- `LiveCheckinScorer` — POST `{API}/api/checkin/score` with optional bearer token.
- `DemoCheckinScorer` — offline heuristic (`scoreCheckinTextLocally`) per answer, same
  artificial delay pattern as `DemoCompanionClient`.
- `scoreCheckinBatch(scorer, req) → { scores: Map<questionId, {points, reason?}>, greeting?
  }` — orchestrates the call; a network failure, a bad response shape, or the model
  dropping a question all fall back to the offline heuristic **per missing item**, never
  leaving a question unscored. `reason`/`greeting` are simply absent on any fallback path —
  their presence is itself the live-vs-offline signal.
- **Point + reason (2026-07-20, verification aid only).** The live LLM response includes a
  one-sentence `reason` per scored question — **never rendered in the UI**, only
  `console.log`ged from `renderCheckin`'s `finish()` — specifically so it can't run afoul
  of the "no assessment language in front of the child" rule `tests/e2e/child-surface.spec.ts`
  guards. Open devtools during a logged-in check-in to see it.
- **Personalized greeting (2026-07-20).** The same batch call also asks the model for one
  short, warm hello in the companion's voice, referencing the child's actual answers by
  content and addressing them by name (`CheckinScoreRequest.childName` —
  `gameState.profile.childDisplayName`, see the `childrenScreen.ts` note in §3.5 for how a
  real name gets there). Real child-facing free text, so unlike the points-only contract
  this route originally shipped with, the greeting **is run through `filterOutput`**
  server-side; an unsafe/missing greeting is simply omitted and the client falls back to
  the pre-existing authored `checkinCompanionLine(placement, companionName)`. Shown on the
  results screen (`renderResult()`) with its own emphasized `.checkin-greeting` style
  (`global.css`) when present — this is the whole point of the feature, so it's designed to
  read as the headline moment, not blend into ordinary copy.
- Wired in `renderCheckin` (`src/ui/screens.ts`): tapped answers are unchanged (instant,
  deterministic points); a typed answer runs `checkTypedSafety` **immediately** on submit
  (distress/blocked → instant fallback points, same as before) and, if clean, queues into
  `pendingTyped` instead of scoring right away. Once all questions are answered, if
  anything is pending, a brief "{companion} is thinking about what you shared…" card shows
  while the one batch call resolves, then the existing compass/placement screen renders
  (now possibly with the live greeting) — `buildCheckinResult`'s math never changed, only
  how a typed answer's points are produced.
- **Manually verified live, end to end** (2026-07-20): created a throwaway pre-confirmed
  Supabase user via the admin API, signed in for a real access token, called
  `/api/checkin/score` with two distinct typed answers — got back per-question `reason`s
  that correctly explained the specific wording, and a `greeting` that named the child,
  referenced both answers' actual content, and cleanly passed `filterOutput` (~2.2s
  round-trip, vs <35ms for the unauthenticated fallback). Test user deleted afterward.
...
- `checkin.ts` — pre-level check-in: `CHECKIN_QUESTIONS` bank (6 open-ended questions,
  ...
  deterministic rotation), `buildCheckinResult` (0–10 starting point, bright/steady/gentle
  placement; answer text never stored), placement labels/companion lines.
  `buildJourneyReflection` appends the baseline to its summary + parent coaching. Also
  hosts the distress-resume helpers (spec §17D): `shouldResumeInDistress(flag)` predicate +
  `RESUME_DISTRESS` re-entry copy (**SME-draft, sign-off pending**), consumed by `main.ts`'s
  boot branch and `renderResumeCheckin`.
  **Typed-answer scoring split 2026-07-20** (first LLM-backed check-in scoring — see
  [safety-companion-pipeline.md](./docs/context/safety-companion-pipeline.md)):
  `checkTypedSafety(raw)` runs `filterInput` **immediately** on submit (distress/blocked →
  instant fallback points, never deferred to a network call) and `scoreCheckinTextLocally(text)`
  is the pure keyword scorer (extracted, same word lists) — both are the single source of
  truth shared with the server's LLM-scoring fallback. `scoreTypedCheckinAnswer` (offline/demo
  path, tests unchanged) is now built on top of these two.
...
- `routes/companion.ts` — `POST /api/companion`: input filter → **auth gate** (2026-07-20:
  Anthropic only called on a verified Supabase-logged-in session — `verifySupabaseToken`
  on the `Authorization: Bearer` header; missing/invalid token behaves exactly like no
  `ANTHROPIC_API_KEY`) → Anthropic (if key + authed) or local **rubric score**
  (`scoreTypedResponse` via `req.typedRubricRef`, §3.9) → confidence floor → output filter
  → fallbacks library; `POST /api/reflect`: event list → counselor insights (no client
  caller yet).
- `routes/checkin.ts` — **new (2026-07-20).** `POST /api/checkin/score`: takes the whole
  pre-level check-in's typed answers as one batch (only answers that already passed
  `checkTypedSafety` client-side) plus the optional `childName`; re-runs `filterInput` per
  answer server-side anyway (defense in depth — a blocked one is scored via the same
  fallback rule and excluded from the model call); same **auth gate** as `/api/companion`
  above — no key, no valid logged-in session, parse failure, or timeout all →
  `scoreCheckinTextLocally` (§3.9) for whichever answers still need it, no `greeting`; else
  one Anthropic call producing `{"scores":[{"questionId","points":0-2,"reason"}],"greeting"}`
  — every score falls back to the offline heuristic per-item on drop/invalid-range, same as
  before. **`greeting` is real free child-facing text (2026-07-20 addition)** — unlike the
  points-only contract this route originally shipped with, it now goes through
  `filterOutput` before ever leaving the server; unsafe or missing → simply omitted (`§3.6a`
  covers the client's fallback). `reason` is truncated (200 chars) but never filtered — it's
  a dev-console-only verification aid, never rendered. Guarded by `tests/unit/authGate.test.ts`
  (runs against whatever real key is in `.env`, no mocking) and `tests/unit/checkinScore.test.ts`
  (`parseModelResponse`'s validation/fallback/output-filter matrix). See
  [safety-companion-pipeline.md](./docs/context/safety-companion-pipeline.md).
...
| [safety-companion-pipeline.md](./docs/context/safety-companion-pipeline.md) | `server/routes/companion.ts`, `server/routes/checkin.ts`, `src/companion/*`, `src/counselor/checkin.ts`, `src/safety/*`, fallbacks | Live vs demo companion paths, filters, fallback coverage, + the check-in batch LLM scorer (2026-07-20) |
...
> feedback), now pushed and CI-proven.** Landed across this arc (through local commit
> `a04938c`, **now on `origin/main`** — see NEXT-5 #5 below): killed the in-game assessment leak (child no longer sees coach/clinical
...
> >    **→ next real task is #5.**
> > 5. ✅ **Verify CI actually runs** — done 2026-07-20. Pushed all 34 local commits to
> >    `origin/main` (first push since the repo-root workflow relocation); GitHub rejected
> >    the first attempt (`refusing to allow an OAuth App to create or update workflow
> >    ci.yml without workflow scope`) until the `gh` token was re-authed with the
> >    `workflow` scope. Once pushed, run `29769482514` went green end-to-end in 1m29s —
> >    typecheck, lint, validate:content, test:unit, build, audit:bundle, and the full
> >    Playwright e2e suite (incl. the projector suite) all passed on a real GitHub Actions
> >    runner, not just locally. Every prior "in CI" claim in this ledger is now actually
> >    proven, not just configured. **→ remaining Phase-4 items are the human ones:
> >    screen-reader pass, backup video, scope freeze.**
>
...
| ⚠️ **The CI workflow was never running on GitHub.** It lived at `trunorth/.github/workflows/ci.yml`, but GitHub Actions only discovers workflows under the **repo-root** `.github/workflows/`. **Confirmed against the repo's Actions tab on 2026-07-19: zero workflows, zero runs** — so every "in CI" claim previously made in this ledger (lint, red-team, e2e, bundle budget) was *configured* but never once enforced on a push | (unassigned) | ✅ fixed 2026-07-19 (Phase 4), **verified 2026-07-20** — moved to repo-root `.github/workflows/ci.yml` with `defaults.run.working-directory: trunorth` (there is no root `package.json`, so `npm ci` would otherwise fail on step 1) and `cache-dependency-path: trunorth/package-lock.json`. Pushed 34 local commits to `origin/main` (first push had to be re-run after re-authing the `gh` token with the `workflow` scope — GitHub rejects any push touching `.github/workflows/*` without it). Run [`29769482514`](https://github.com/Madhu040/DallasAITeam15/actions/runs/29769482514) went green in 1m29s: typecheck, lint, validate:content, test:unit, build, audit:bundle, `playwright install`, and the full e2e suite (19 tests incl. the projector suite) all passed on a real runner. No longer a "configured but unproven" claim |
...
| **Decision overlay does not scale with resolution** (surfaced by the projector suite, 2026-07-19) — the choice button measures **452×64px / 16px text at 1024×768, 1366×768 and 1920×1080 alike**, unlike characters/HUD which scale with `--px`. It sits exactly on the §17A.4 64px floor everywhere, so it passes, but on a large venue projector the most important surface in the demo is proportionally the smallest it ever gets — cuts against §13A.4 "readable from 50 feet" | (unassigned) | ✅ fixed 2026-07-20 — `.choice-panel`/`.choice-panel h2`/`.choice-btn`/`.typed-input`/`.typed-submit` now use `clamp(floor, calc(N * var(--px-equivalent)), ceiling)`, the same pattern characters/HUD already used, with the floor pinned to the real §17A.4 minimum (64px hit target, 16px text) so it can never regress below the accessibility line, and a ceiling so it grows on a bigger stage instead of sitting fixed. **Real architectural snag found mid-fix:** `--px` is `cqw`-based, which only resolves on DOM *descendants* of `.game-viewport` (the element with `container-type: size`) — but the decision overlay (like every overlay in the app — dialog, celebration, parent gate…) is mounted as a *sibling* of `.game-viewport` via `container.appendChild(overlay)` in `GameView.ts`, not inside it. `var(--px)` silently failed to resolve there (an invalid `calc()` drops the whole declaration), which is exactly how the original bug shipped invisibly — the first attempt at this fix reused `--px` directly and *measurably did nothing* (button height came back 26px, below even the old fixed value). Rather than restructure DOM mounting for every overlay in the app, added a `:root`-scoped `--overlay-px: calc(min(100vw, 100vh * 16/9) / 1920)` — mathematically identical to `--px` (it reproduces `.game-viewport`'s own sizing formula in viewport units instead of container-query units), inherits everywhere with no container needed. Also fixed `.typed-submit`'s `min-height: 48px`, a pre-existing §17A.4 floor violation independent of the scaling bug (64px is the Ch.1-band requirement). **Deleted** a narrow-viewport (`max-width:600px`) override that forced 14px text / 48px hit-target — both below spec floors — now redundant and actively wrong once the base `clamp()`s correctly floor-out at any width on their own. New e2e assertion (`projector.spec.ts`) proves the fix rather than just floor-compliance: choice-button width must be ≥1.3× larger at 1920×1080 than at 1024×768 (measured 266.7px → 500px, exactly matching the `500 * (viewportWidth/1920)` formula) — a floor-only check can't tell "scales correctly" apart from "sits fixed exactly at the floor," which is precisely how the original defect shipped passing. Verified visually at both extremes in-browser: clean, centered, no overflow. 19 e2e (18→19) + 169 unit green; typecheck/lint/build clean |
...
| **Mobile / touch device support** — scoped 2026-07-20, not yet built. `src/input/InputController.ts` only listens for `keydown`/`keyup` (WASD/arrows, E/Space/Enter) — zero touch handling, so a phone tapping the screen does nothing today. Confirmed the stage's `--px`/`--overlay-px` scaling (`cqw`-based, `min(100vw, 100vh*16/9)`) is architecturally resolution-independent and *should* scale down to phone widths the same way it already proved out from 1024×768 to 1920×1080 for the projector fix — but this has never actually been measured below 1024px wide. No orientation handling exists (16:9 stage on a portrait phone renders as a small letterboxed strip; no rotate-prompt). Audio autoplay on iOS Safari's gesture requirement is unverified on a real device (`sfx.ts`'s `.play().catch(() => {})` fails silently either way, so it won't crash, but ambience may just never start). Zero e2e coverage at mobile viewports/touch emulation. Plan: (1) add a touch input layer to `InputController` feeding its existing `getMoveVector()`/`consumeInteract()` interface, gated behind `matchMedia('(pointer: coarse)')` so desktop is untouched; (2) portrait rotate-prompt, same media-query gating; (3) measure `--px` scaling at real phone widths (375/390/412px); (4) tie the first audio call to the same tap that starts the game; (5) add a mobile Playwright project. None of this touches `WorldRuntime`, rendering, or content | (unassigned) | ⬜ scoped, not started |
...
| Date | Change |
|---|---|
| 2026-07-23 | **Closed a third caption gap — the check-in results screen ("the wording that comes after the initial questionnaire") never showed a caption at all, because it lives in `screens.ts`, a completely separate screen from `GameView.ts` that the original captions work never touched.** `renderResult()` now calls `showCaption(companionName, lineText, true)` right where it sets the placement line/LLM greeting text (persistent — this screen stays up until the child taps "Start", the same kind of stable "still speaking" condition as a stage-object dialog, not the timer-based bubble case), with an explicit `clearCaption()` added to the "Start" button's handler before `onDone(result)` fires so nothing carries over into gameplay. Verified in-browser: answered the 3-question check-in (one typed answer), confirmed the results screen's caption showed the exact placement line, held for the full ~23s of manual interaction with **no premature timeout** (proving the `persistent` path fired, not the 4.5s timer path), and cleared the instant "Start" was clicked. **Aside, not a code change:** earlier the same day, "I can't hear the person talking" turned out to be the companion voice toggle (🔊/🔇, top HUD) sitting switched off in this browser's `localStorage` — almost certainly flipped by this session's own automated testing clicking around the HUD icons while verifying captions, not a regression. Fixed by setting `trunorth_voice_enabled` back to `"1"`; confirmed live via `speechSynthesis.speaking` going `true` on a real scene line. Worth remembering if a future session's automated testing needs to click near that button again. 197/197 unit tests, lint, typecheck, all 19 e2e specs, bundle audit all green (rebuilt `dist/`). |
| 2026-07-21 | **Repositioned speech captions bottom-right → bottom-center per a second "still not showing" report, restyled lighter (small translucent pill instead of a card), and re-verified from scratch rather than assuming yesterday's fix generalized.** CSS only (`global.css` `.speech-captions`): `left: 50%; transform: translateX(-50%); bottom: 28px`, smaller font (14px), fully rounded (`border-radius: 999px`), lighter background (`rgba(15,15,20,0.5)` vs the previous card), centered text — reads as a subtitle, not a competing panel, addressing the "make sure it's not too distracting" ask alongside the position change. No logic changes — the `persistent`/timer mechanism from the same-day-earlier fix (previous row) was untouched. **Re-verification, done more rigorously than the previous round:** `localStorage.clear()` + a full fresh reload (ruling out any stale saved-session state or browser cache as an explanation for "still not showing"), replayed landing → trust screen → onboarding → hub → Everbright Meadow → skip check-in → trigger a decision, with a `MutationObserver` attached *before* the first interaction (not just around one isolated click) so the entire session's caption activity was captured. Confirmed two distinct spoken lines in sequence both correctly captioned at the new position: the scene-opening companion line ("Hey Nova — someone's crying over by the bench...") and the decision's consequence line ("You noticed someone feeling alone...", held ~5s then auto-hid on schedule) — screenshotted directly at the bottom-center position as visual confirmation, not just DOM-state assertions. Given this is the second "not showing" report despite a verified-working fix, flagged to the user that a stale/cached tab from before the fix (needing a hard refresh) or a different testing path (e.g. a build/deploy not yet carrying these changes) are plausible explanations if it still doesn't appear on their end, since local reproduction keeps succeeding. 197/197 unit tests, lint, typecheck, all 19 e2e specs, bundle audit all unaffected/green (rebuilt `dist/`). |
| 2026-07-20 | **Fixed the speech-captions box (previous row) — it worked for stage-object dialogs but was effectively invisible for the companion bubble, root-caused via `MutationObserver` timestamps rather than guessed at.** User report: "captions aren't showing." First debugging pass (rapid `setTimeout`-polling the element's class from devtools) looked like the bug was still present *after* an initial timer-based fix too — turned out to be a testing-methodology artifact (polling started too many real seconds after the click, by which point a correctly-working timer had already legitimately expired), not a second bug; a `MutationObserver` recording every class-attribute change with real timestamps gave the actual, trustworthy timeline instead of a guess. **Real root cause:** the shipped version cleared the caption on any `renderGameView` render that didn't currently show a bubble/dialog — correct for dialogs (a stable, checkable condition across renders) but wrong for the companion bubble, because `SceneEngine.resolveDecision` sets phase `consequence` and then, **synchronously with no intervening delay**, `exploring` (same function, nothing awaited but a `store.save`) — so the one render pass where the bubble's condition was briefly true could last single-digit milliseconds, often already gone by the very next render, nowhere near long enough to read. **Fix:** `showCaption(speaker, text, persistent?)` gained a `persistent` flag and an internal `setTimeout`. Non-persistent (the bubble, default) starts a timer using new `appConfig.timing.captionMinVisibleMs` (default 4500ms, env `VITE_CAPTION_MIN_VISIBLE_MS`) that's the actual floor on how long a line stays up, decoupled from the phase's real (much shorter) lifetime; a new call resets it. Persistent (stage-object dialogs, called with `showCaption(speaker, pageText, true)`) skips the timer — the dialog staying open genuinely is "still speaking," so it's cleared explicitly instead, from a new `clearCaption()` call in `main.ts`'s `closeDialog()`. The old per-render `captionSpoken` boolean and the blanket `if (!captionSpoken) clearCaption()` at the end of `renderGameView` were **removed entirely** (that was the bug, not a redundant safety net) — `renderGameView` no longer touches `clearCaption` at all. `main.ts`'s `navigate()` keeps its existing `clearCaption()` call for leaving the game screen. **Verified properly this time** — `MutationObserver`-instrumented the live element's `class` attribute with `Date.now()` timestamps across a fresh, from-scratch playthrough (cleared `localStorage`, replayed landing → trust → onboarding → hub → decision): the bubble caption's `visible` class was added at t≈0 and held for the full ~4.5s window before auto-clearing (confirmed by the actual recorded timestamps, not a screenshot glance); separately, the dialog caption held past 7s while the dialog remained open on-screen and cleared the instant "Done" was clicked. Every change here is DOM/timing behavior with no new pure logic, so — consistent with the original entry — verification stayed end-to-end in the browser; 197/197 unit tests, lint, typecheck, and all 19 e2e specs (rebuilt `dist/`) still green, bundle audit unaffected. |
| 2026-07-20 | **Added persistent, translucent, bottom-right speech captions — shown whenever a character speaks, independent of the TTS voice toggle.** New `src/ui/captions.ts`: `showCaption(speaker, text)`/`clearCaption()`, backing a single `div` appended once directly to `document.body` (not inside `#app`, since every screen's render fully tears down `#app`'s contents — a node there would be destroyed on the very next re-render, and `renderGameView` in particular rebuilds on every phase/meter update). Deliberately **not** hooked into `speakLine` itself (`src/audio/speech.ts` early-returns when the voice toggle is off) — captions are meant to work whether or not TTS audio is enabled, so they're called independently from the same two places a character's line already becomes visible: `GameView.ts`'s companion speech bubble (`showCaption(state.profile.companionName, companionLine)`) and `renderDialogOverlay`'s per-page NPC/companion dialog text (`showCaption(speaker, pageText)`, right alongside the existing `speakOverlayOnce` call). A new per-render `captionSpoken` boolean in `renderGameView` triggers `clearCaption()` whenever neither source fired that render, so the box never lingers once nothing is actually being said; `main.ts`'s `navigate()` also clears it when leaving the game screen, alongside the pre-existing `stopSpeaking()` call. **Deliberately scoped out:** the decision/discuss-prompt read-aloud scripts (`buildOverlayScript`, `discussPrompt`) — these are TTS-only accessibility narration of content that's already fully visible as structured UI (a heading + option buttons), so captioning the synthesized script would just duplicate it as a wall of text rather than read as "a character speaking a line." CSS (`global.css`): `position: fixed; right: 16px; bottom: 96px` (offset above the counselor panel's default `bottom: 12px` so the two don't usually stack), `pointer-events: none` (never blocks clicks/taps under it), translucent dark background so the scene stays visible through it, `z-index: 101` (above the modal `.overlay` layer at 100, so a caption stays legible even during a dialog pop-up). **Verified in-browser via Chrome automation** (not just unit tests, since this is a purely visual feature): played through the offline demo (`?demo=1`) into a stage-object dialog and confirmed via both screenshot and a direct DOM read that the caption showed the exact speaker + page text (`"Buddy: Someone stacked these to show the way for whoever came next."`), correctly updated to the second page's text on "Next," and cleared the instant "Done" was clicked. All 19 e2e specs still green (including the axe accessibility scans, which pass cleanly with the new `aria-live="polite"` region) and 197/197 unit tests, lint, and typecheck unaffected — this feature has no unit-testable pure logic of its own (it's DOM/CSS), so verification was end-to-end in the browser rather than a new test file. |
| 2026-07-20 | **Check-in scoring now also returns a personalized LLM greeting (by the child's actual name, referencing what they shared) plus a per-question "reason" — console-only verification aid, never rendered to the child.** `POST /api/checkin/score`'s contract changed from a plain JSON array to an object: `{"scores":[{"questionId","points":0-2,"reason"}],"greeting"}`. `parseModelResponse` (`server/routes/checkin.ts`) rewritten to parse the object, validate/truncate `reason` (200 chars, never filtered — it's dev-console-only, not child-facing), and run the new `greeting` through `filterOutput` (200 chars, real child-facing free text this time, unlike the points-only contract this route shipped with a day earlier) — an unsafe or absent greeting is simply omitted, client falls back to the pre-existing `checkinCompanionLine`. System prompt rewritten to ask for both in one shot (same single Anthropic call, no extra request), with the same boundary language as the companion pipeline (no clinical terms, no identity-claiming, no scores mentioned to the child). Same auth gate as before applies — greeting only exists on a logged-in, authenticated, key-configured session. **Closed a real personalization gap along the way:** `gameState.profile.childDisplayName` defaults to the hardcoded placeholder `"Nova"` for every solo session (`createInitialGameState`) and was never actually set from a real child profile — the `childrenScreen.ts` "Continue" button (shipped a day earlier) was disconnected from gameplay state entirely. Fixed by adding a **"Play as {name}"** button per child row (`renderChildren`'s `onContinue` now takes an optional `childName`); `main.ts`'s `"children"` case sets `gameState.profile.childDisplayName` from it before navigating to hub. `renderCheckin` gained a `childName` parameter threaded through from there into the scoring request. Client: `scoreCheckinBatch` (`CheckinScorer.ts`) return type changed from a plain `Map<questionId, points>` to `{ scores: Map<questionId, {points, reason?}>, greeting? }`; `renderCheckin`'s `finish()` now `console.log`s each question's point + reason (dev-only — deliberately never touches the DOM, so it can't violate the "no assessment language in front of the child" rule `tests/e2e/child-surface.spec.ts` guards) and stores the greeting for `renderResult()`, which shows it in a new emphasized `.checkin-greeting` style (`global.css`) in place of the generic placement line whenever present. **Manually verified fully live, not just against the fallback path this time:** created a throwaway pre-confirmed Supabase test user via the Auth admin API (signup alone wasn't enough — this project has email confirmation on), signed in for a real access token, called `/api/checkin/score` with two genuinely different typed answers — got back specific, correct per-question reasons and a greeting that named the child and referenced both answers' actual content, cleanly passing the output filter (~2.2s round-trip, confirming Anthropic was truly called, vs the <35ms unauthenticated fallback). Test user deleted afterward via the same admin API — zero leftover test accounts in the project. Updated `tests/unit/checkinScore.test.ts` (11 tests, was 6 — added the object-parsing/greeting-filtering matrix) and `tests/unit/checkinScorer.test.ts` (7 tests, was 4) for the new contract; `tests/unit/authGate.test.ts`'s assertions still hold unchanged (the auth-gated fallback response shape is a subset of the new one). 197/197 unit tests, lint/typecheck/validate:content clean, all 19 e2e specs green (including `child-surface.spec.ts`, the one that would catch a reason/greeting leak into child-facing DOM), bundle audit clean (no new dependencies). |
| 2026-07-20 | **Gated the Anthropic API key behind a logged-in parent session — cost/abuse control, applies to both `/api/companion` and `/api/checkin/score`.** Both routes now read the `Authorization: Bearer` header and run it through `verifySupabaseToken` (already built for the Supabase auth migration); no header, an invalid/expired token, or no `ANTHROPIC_API_KEY` at all are now the exact same branch — straight to the existing offline scorer (`scoreTypedResponse`/`scoreCheckinTextLocally`). Purely a server-side change: `LiveCompanionClient` and `LiveCheckinScorer` already took an optional bearer token and were already passing `getToken()` through, so no client code needed to change — the server just wasn't checking it before. Input-safety filtering (`filterInput`/distress detection) is entirely unaffected — it always runs first, regardless of auth state. **Practical consequence, stated plainly:** "Play Now (Guest)" — the primary, most common way a child plays, per the product's own guest-first design — never triggers a live Claude call under this rule; only sessions reached via parent login get live-scored companion feedback and check-in scoring, and guest play now always gets the same offline-heuristic experience regardless of whether a key is configured. New `tests/unit/authGate.test.ts` (3 tests) — deliberately runs against whatever real `ANTHROPIC_API_KEY` is sitting in the local `.env` rather than mocking the SDK, so a future regression would either time out or return a live-shaped response and fail the assertions, instead of silently passing. **Manually verified against a real Anthropic key** (curl, no Authorization header and a garbage Bearer token on both routes): both returned in <35ms with the exact deterministic offline-heuristic output, confirming Anthropic was never actually reached. 188/188 unit tests, lint/typecheck/validate:content clean, all 19 e2e specs green, bundle audit clean. Docs: [safety-companion-pipeline.md](./docs/context/safety-companion-pipeline.md) updated with the gate + its consequence. |
| 2026-07-20 | **First Anthropic/LLM-backed scoring path: the pre-level check-in's typed ("own words") answers can now be scored by Claude instead of only the offline keyword heuristic — output contract is still the existing 0/1/2 points per question, unchanged math downstream.** Mirrors the existing `/api/companion` DP-scoring architecture (direct `Anthropic.messages.create()` call, not an agent/tool loop), but scores the **whole check-in as one batch call at the end** rather than per-question, since the check-in UI already shows no per-question feedback. New server route `server/routes/checkin.ts` — `POST /api/checkin/score`: re-runs `filterInput` per answer server-side (defense in depth, never trusts the client), blocked answers get the existing fallback point rule (distress→0, other blocks→1) and are excluded from the model call; no `ANTHROPIC_API_KEY` / parse failure / timeout → `scoreCheckinTextLocally` (extracted pure keyword scorer, single source of truth shared with the offline path); else one Claude call with a strict `[{"questionId","points":0|1|2}]` JSON-array contract, `parseModelResponse` validates + clamps + falls back to the offline heuristic **per-item** for anything dropped/invalid — a question is never left unscored. No free-text output, so no `filterOutput` surface. New client `src/companion/CheckinScorer.ts` (`LiveCheckinScorer`/`DemoCheckinScorer` mirroring `CompanionClient`'s split, `scoreCheckinBatch` with the same graceful per-item fallback). `src/counselor/checkin.ts` refactored (behavior-preserving, existing tests pass unchanged): extracted `scoreCheckinTextLocally` (pure keyword scorer) and `checkTypedSafety` (filter-only check, distress/blocked resolved **instantly on submit** — safety detection is never deferred to the batch network call) out of `scoreTypedCheckinAnswer`. `renderCheckin` (`src/ui/screens.ts`) rewired: tapped answers unchanged (instant); a typed answer runs `checkTypedSafety` immediately (blocked → instant fallback points as before), a clean one queues into `pendingTyped`; once all questions are answered, if anything is pending, a brief "{companion} is thinking about what you shared…" card shows while the one batch call resolves, then the existing compass/placement screen renders — `buildCheckinResult`'s math untouched. `renderCheckin`'s signature gained `companionArchetype`/`ageBand` params (prompt context); `main.ts`'s one call site updated. **Verified:** 185/185 unit tests (12 new — extracted-function coverage in `checkin.test.ts`, new `checkinScore.test.ts` for `parseModelResponse`'s validation/fallback matrix, new `checkinScorer.test.ts` for `scoreCheckinBatch`'s network-failure/partial-response fallback), lint/typecheck/validate:content clean, all 19 e2e specs green (only `demo-mode.spec.ts` touches the check-in screen and only via tapped options, so the typed/LLM path wasn't in scope for e2e), `audit:bundle` clean (new module is tiny, no budget impact), manual curl verification of `/api/checkin/score`: no-key fallback matches the offline heuristic exactly, a distress answer is caught and scored 0 even without the client-side filter running first, and an empty batch correctly 400s. **Not verified live:** no `ANTHROPIC_API_KEY` was configured in this session, so only the no-key fallback path was exercised end-to-end — the actual Claude-scored path needs a real key to confirm the prompt/parsing round-trip against a live response. |
...
| 2026-07-20 | **Pushed to GitHub and proved CI actually runs — the first real test of the repo-root workflow relocation.** All 34 local commits (through the ledger-sync commit `a04938c`) went to `origin/main` for the first time since `ci.yml` moved from `trunorth/.github/workflows/` to the repo root. The push was rejected on the first attempt — `refusing to allow an OAuth App to create or update workflow ci.yml without workflow scope` — because the `gh` CLI's cached token only had `gist`/`read:org`/`repo`; GitHub requires the `workflow` scope specifically for any push touching `.github/workflows/*`, which this push did. Re-authed (`gh auth refresh -h github.com -s workflow`, user approved via device code), then the push went through cleanly. Watched the resulting run ([`29769482514`](https://github.com/Madhu040/DallasAITeam15/actions/runs/29769482514)) to completion rather than assuming a queued run would pass: green in 1m29s, every step in order — `npm ci`, typecheck, lint, validate:content, test:unit, build, audit:bundle, `playwright install --with-deps chromium`, and the full `test:e2e` suite (19 tests, including the projector suite that only exists because of this same session's earlier work). A few `tsc` "File … is not under rootDir" annotations surfaced during the build step (the pre-existing `tsconfig.server.json` quirk CLAUDE.md already documents — stray compiled `.js` alongside `.ts` sources) but did not fail the job. This closes out NEXT-5 item 5 and the standing "unverified until someone pushes" caveat on the CI gap-table row: every "in CI" claim made throughout this whole arc is now backed by one real green run on GitHub, not just a passing local script. Docs-only change (this changelog entry + the NEXT-5/gap-table updates) — no application code touched. |
| 2026-07-20 | **The decision overlay now scales with the projector, closing a gap the projector e2e suite surfaced.** The choice button was fixed-px (452×64/16px) at every one of the three tested resolutions, unlike characters/HUD which already scale via `--px`; harmless at the venue's exact reference resolution, but the single surface the audience actually watches would shrink proportionally on any bigger screen. Converted `.choice-panel`/`h2`/`.choice-btn`/`.typed-input`/`.typed-submit` to `clamp(§17A.4-floor, calc(N * scale-var), ceiling)`. **Real snag along the way:** `--px` is `cqw`-based and only resolves on DOM descendants of `.game-viewport`; every overlay in the app (decision, dialog, celebration, parent gate…) mounts as a *sibling* of it instead, so reusing `--px` directly silently failed (an invalid `calc()` drops the whole declaration) — confirmed by measuring the button *smaller* than before after the first attempt, not just unchanged. Fixed with a `:root`-scoped `--overlay-px: calc(min(100vw, 100vh*16/9) / 1920)` that reproduces `.game-viewport`'s own sizing formula in viewport units — mathematically identical to `--px`, but inherits from anywhere with no container needed, so it didn't require moving every overlay's DOM mount point (a much larger, riskier change touched to fix one flagged row). Also fixed `.typed-submit`'s `min-height: 48px` (a pre-existing floor violation, unrelated to scaling — 64px is the spec minimum for the Ch.1 band) and **deleted** a narrow-viewport override that forced 14px text/48px hit-target, both already below spec floors and now redundant now that the base rule floors out correctly on its own. New e2e assertion proves genuine scaling rather than floor-compliance (a fixed-px button sitting exactly on the floor passes a floor-only check identically to a scaling one — that ambiguity is how the original bug shipped invisibly): choice-button width must be ≥1.3× larger at 1920×1080 than 1024×768; measured 266.7px → 500px, exactly matching `500 × (viewportWidth/1920)`. Verified visually at both extremes. 18→19 e2e green, 169 unit unaffected; typecheck/lint/build clean. |
