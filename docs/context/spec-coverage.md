...
| Sound design: SFX, ambient bed, event-mapped cues | §17B.4 | ✅ | Shipped 2026-07-20 — all 7 clips landed and wired. Two had defects caught by waveform analysis: `footstep`'s transient landed after its own retrigger interval (never actually audible while walking) and `exploring-bed` didn't loop cleanly (audible click every ~2s); both re-trimmed/faded and re-exported as `.wav` (no MP3 encoder available locally). ⚠️ Ambience is still only ~2s of source material — reads as repetitive under the calm-first goal even with the click fixed; flagged as a regenerate-longer task in `public/audio/README.md`, not blocking |
